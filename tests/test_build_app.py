...
def _install_fakes(procs, changes=None):
    ...
    build._git = fake_git
    build._has_changes = lambda d: scripted.pop(0) if scripted else True
...
def test_all_tasks_succeed(tmp):
    ...
    out = asyncio.run(build.build_app_node(_state(tmp)))
...
def test_missing_prerequisites(tmp):
    out = asyncio.run(build.build_app_node({"build_plan": PLAN, "build_tasks": []}))
...
def test_build_dir_validation(tmp):
    ...
    build._git = lambda d, *a: None

    out = build.choose_build_dir_node({"build_plan": PLAN, "build_tasks": PLAN.tasks})
