...
- `tui/` — Textual app driving the whole pipeline in-process. `app.py` (layout, driver loop, resume, skip), `dialogs.py` (one modal per human-in-the-loop moment), `history.py` (inferring past-run progress from checkpointed state). `AsyncSqliteSaver` in `.runs.db` for resume.
...
- `progress.py` — how the pipeline talks to a UI: `_say`/`set_say_sink` for streamed output, and the skip-flag API (`request_skip`/`clear_skip`/`skip_requested`, `SkippedByUser`).
...
- `progress.set_say_sink(fn)` redirects build output; `None` restores `print`. The TUI routes it through a `queue.SimpleQueue` because LangGraph runs sync nodes in a thread pool, so `_say` is not guaranteed to be on the event loop.
