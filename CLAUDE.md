...
- `cli.py` - plain `print`/`input` driver for the whole pipeline in-process: driver loop, interrupt prompts, resume, and `ls` of past runs. `AsyncSqliteSaver` in `.runs.db` for resume.
- `nodes/` — node functions, one module per phase: `judges.py` (fetch/theme/judges/bias), `ideas.py` (brainstorm/rank/research/select), `build.py` (plan/choose dir/opencode build), `common.py` (shared research-worker policy: timeout, skip, degrade via `run_researcher`). Each phase module owns its model bindings and semaphore.
...
- **Structured output** uses `model.with_structured_output(PydanticModel)` — see `judge_extractor` in `nodes/judges.py` and `idea_generator`/`idea_compiler` in `nodes/ideas.py`. The `Judges`, `IdeaCandidates`, and `FinalIdeas` models each have a `@model_validator(mode="before")` that unwraps bare lists, since some Ollama models return `[...]` instead of `{"judges"/"ideas": [...]}`.
...
uv run python -m agent.cli <devpost-url>   # full pipeline in-process
uv run python -m agent.cli ls              # list past runs
uv run python -m agent.cli --thread <id>   # resume a checkpointed run
...
- **Model calls retry transient network failures.** Ollama Cloud drops connections mid-response (`httpx.ReadError`, usually with an empty message), which killed runs at `rank_ideas` and elsewhere. `resilient()` in `model.py` wraps each bound object with `.with_retry(retry_if_exception_type=TRANSIENT_ERRORS)`. **Wrap the bound objects, not `model`:** `model.with_retry()` returns a `RunnableRetry`, which no longer has `with_structured_output`. The two direct call sites use `judges.chat`, not `model` — patch `chat` when stubbing.
...
- **Retries and degradation only fire on errors, never on silence.** A hung model call stalls a phase forever, so research workers run through `common.bounded_research()` with `RESEARCH_TIMEOUT` (default 300s) and a skip watcher. `progress.request_skip()` abandons in-flight workers and is distinguishable from failure in the placeholder text. Nothing triggers it since the Textual UI was removed; wire it to a key or signal if needed.
...
- **Never decide "the run finished" from `state.next`.** At a node's *second* `interrupt()`, LangGraph reports `state.next == ()` even though the graph is still waiting — so a driver keyed on it declares success mid-run. `choose_build_dir` hits this on any bad path. Key on `__interrupt__` appearing in the `astream(stream_mode="updates")` output instead; `tests/test_cli_driver.py` pins this behaviour.
- `cli.ask` dispatches interrupt prompts on **payload shape, not node name**, because `choose_build_dir` emits two different payloads (path prompt vs. non-empty-directory confirmation).
- **Past-run progress is inferred from state values.** The graph only reports events for the current process, so `completed_phases()` in `cli.py` (used by `ls`) infers finished phases from checkpointed state; `review_bias` is special-cased because it has no output of its own (it overwrites `judge_bias`), so it is inferred from `idea_candidates`.
